...
df_drugs_smiles = pd.read_csv(f'{dir}/drug_smiles.csv')
...
# Max number of atoms m[1] is Mol object of the drug
ATOM_MAX_NUM = np.max([m[1].GetNumAtoms() for m in drug_id_mol_graph_tup])
# Get all symbols of all Atoms
...
# Get all Hydrogens attached to the atom
...
"""
Notes:
MOL_EDGE_LIST_FEAT_MTX: dict(drug_id: (edge_list, feature_matrix))
...
-> row1: Contains the start nodes of each edge. row2: Contains the end nodes of each edge.
...
df_all_pos_ddi = pd.read_csv(f'{dir}/ddis.csv')
